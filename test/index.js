...
function renderStorageContents() {
  document.querySelector('#sessionContents').innerText = JSON.stringify(sessionStorage, null, 2);
  document.querySelector('#localContents').innerText = JSON.stringify(localStorage, null, 2);
}
...
function App() {
  ...
  const [localValueClone, setlocalValueClone] = useLocalStorage('localValue');
  ...
  return <>
    ...
    <div style={{display: 'flex', gap: '1em'}}>
      <div style={{display: 'flex', flexGrow: '1fr', flexDirection: 'column', padding: '1em', border: 'solid 1px #ccc', borderRadius: '1em'}}>
        ...
        <div id='sessionContents' style={{fontFamily: 'monospace', whiteSpace: 'pre-wrap'}}></div>
      </div>
      <div style={{display: 'flex', flexGrow: '1fr', flexDirection: 'column', padding: '1em', border: 'solid 1px #ccc', borderRadius: '1em'}}>
        <strong>localStorage</strong>
        <div id='localContents' style={{fontFamily: 'monospace', whiteSpace: 'pre-wrap'}}></div>
      </div>
    </div>
  </>;
}
